...
static void RunBench(const char* label, int64_t size_bytes, int warmup,
                     int iters, Fn fn) {
  for (int i = 0; i < warmup; i++) fn(size_bytes);
  ...
  double tput_mb     = (static_cast<double>(size_bytes) * iters / (1024.0 * 1024.0))
                     / (elapsed_us / 1e6);

  printf("  [%-4s] %6.0f KB  avg=%8.1f us  p50=%8.1f us  p99=%8.1f us  "
         "tput=%8.1f MB/s\n",
         label, size_bytes / 1024.0, avg_us, p50_us, p99_us, tput_mb);
}
...
int main(int argc, char** argv) {
  ...
  printf("PS Network Benchmark  server=%s:%d  warmup=%d  iters=%d\n\n",
         FLAGS_host.c_str(), FLAGS_port, FLAGS_warmup, FLAGS_iters);

  printf("LoadFakeData  (download: server -> client)\n");
  for (int64_t sz : kSizes)
    RunBench("load", sz, FLAGS_warmup, FLAGS_iters,
             [&](int64_t n) { client.LoadFakeData(n); });

  printf("\nDumpFakeData  (upload: client -> server)\n");
  for (int64_t sz : kSizes)
    RunBench("dump", sz, FLAGS_warmup, FLAGS_iters,
             [&](int64_t n) { client.DumpFakeData(n); });
